...
<xsl:function name="ahf:getAttributeSet" as="attribute()*">
    ...
    <xsl:for-each select="tokenize($normalizedAtrSetName, '[\s]+')">
    	...
    	<xsl:if test="not ($glStyleDefs/*[name()=$elementName])
                  and not ($glAltStyleDefs/*[name()=$elementName])">
    	...
    	<xsl:choose>
    		<xsl:when test="$glAltStyleDefs/*[name()=$elementName]">
    			<!-- search main and alternative fo style definition file -->
    			<xsl:sequence select="$glStyleDefs/*[name()=$elementName]/@*"/>
    			<xsl:sequence select="$glAltStyleDefs/*[name()=$elementName]/@*"/>
    		</xsl:when>
    		<xsl:otherwise>
    			<!-- otherwise search main fo style definition file -->
    			<xsl:sequence select="$glStyleDefs/*[name()=$elementName]/@*"/>
    		</xsl:otherwise>
    	</xsl:choose>
    </xsl:for-each>
...
<xsl:function name="ahf:getCssStyle" as="xs:string">
	...
	<xsl:if test="not($glStyleDefs/*[name()=$prmAttrSetName])
              and not($glAltStyleDefs/*[name()=$prmAttrSetName])">
		<xsl:call-template name="errorExit">
			<xsl:with-param name="prmMes">
    ...
    <xsl:variable name="styles" select="if ($glAltStyleDefs/*[name()=$prmAttrSetName]) then ($glStyleDefs/*[name()=$prmAttrSetName]/@*) else ($glStyleDefs/*[name()=$prmAttrSetName]/@*)" as="attribute()*"/>
...
<xsl:function name="ahf:getAttributeValue" as="xs:string">
	...
	<xsl:choose>
		<xsl:when test="$glAltStyleDefs/*[name()=$prmAttrSetName]/attribute::node()[name()=$prmAttrName]">
			<xsl:sequence select="string($glAltStyleDefs/*[name()=$prmAttrSetName]/attribute::node()[name()=$prmAttrName][1])"/>
		</xsl:when>
		<xsl:otherwise>
			<xsl:sequence select="string($glStyleDefs/*[name()=$prmAttrSetName]/attribute::node()[name()=$prmAttrName][1])"/>
		</xsl:otherwise>
	</xsl:choose>
</xsl:function>
...
<xsl:function name="ahf:getVarValue" as="xs:string">
    ...
    <xsl:choose>
    	<xsl:when test="not ($glVarDefs/*[name()=$prmVarName])
                    and not ($glAltVarDefs/*[name()=$prmVarName])">
    		<xsl:call-template name="errorExit">
    			<xsl:with-param name="prmMes">
		...
		<xsl:otherwise>
			<xsl:sequence select="string($glVarDefs/*[name()=$prmVarName][1]/text())"/>
		</xsl:otherwise>
	</xsl:choose>
</xsl:function>
...
<xsl:function name="ahf:getInstreamObject" as="element()*">
	...
	<xsl:if test="not ($glInstreamObjects/*[name()=$prmObjName])
              and not ($glAltInstreamObjects/*[name()=$prmObjName])">
		<xsl:call-template name="errorExit">
			<xsl:with-param name="prmMes">
	...
	<xsl:choose>
		<xsl:when test="$glAltInstreamObjects/*[name()=$prmObjName]">
			<xsl:sequence select="$glAltInstreamObjects/*[name()=$prmObjName]/*"/>
		</xsl:when>
		<xsl:otherwise>
			<xsl:sequence select="$glInstreamObjects/*[name()=$prmObjName]/*"/>
		</xsl:otherwise>
	</xsl:choose>
</xsl:function>
