...
<!-- Key definition -->
<xsl:key name="KEY_INDEX_START" match="*[contains(@class, ' topic/indexterm ')]" use="@start"/>
<xsl:key name="KEY_INDEX_END"   match="*[contains(@class, ' topic/indexterm ')]" use="@end"  />
...
<xsl:template match="*[contains(@class, ' topic/indexterm ')]" mode="MODE_PROCESS_INDEXTERM_MAIN">
    ...
    <xsl:variable name="startAttrCheck" as="xs:string">
        <xsl:call-template name="indexStartAttrCheck">
            ...
            <xsl:with-param name="prmCurrentIndexKey" select="$currentIndexKey"/>
            <xsl:with-param name="prmDoCheck" select="boolean($prmMakeKeyAndStart or $prmNormalProcessing)"/>
        </xsl:call-template>
    </xsl:variable>
    ...
    <xsl:choose>
        <xsl:when test="string(@end) and $prmDoCheck">
            ...
            <xsl:choose>
                ...
                <xsl:when test="count($correspondingStartIndexterm)&gt;1">
                    <xsl:call-template name="warningContinue">
                        <xsl:with-param name="prmMes" 
                         select="ahf:replace($stMes641,('%end','%key','%file'),(@end,$prmCurrentIndexKey,@xtrf))"/>
                    </xsl:call-template>
                    <!-- Temporary solution. DITA-OT BUG:2891736 -->
                    <!--xsl:value-of select="$INDEX_CHECK_IGNORE_ELEMENT"/-->
                    <xsl:value-of select="$INDEX_CHECK_OK"/>
                </xsl:when>
...
<xsl:template name="indexBothAttrCheck">
    ...
    <xsl:choose>
        ...
        <xsl:when test="string(@start) and string(descendant::*[contains(@class, CLASS_INSDEXTERM)][string(@end)]/@end)">
        ...
        <xsl:when test="string(@end) and string(descendant::*[contains(@class, CLASS_INSDEXTERM)][string(@start)]/@start)">
...
<xsl:function name="ahf:findIndextermEnd" as="xs:boolean">
    ...
    <xsl:choose>
        ...
        <xsl:otherwise>
            ...
            <!--xsl:message>ahf:findIndextermEnd $prmEnd="<xsl:value-of select="$prmEnd"/>" class="<xsl:value-of select="$prmRangeElem/@class"/>" exist="<xsl:value-of select="key('KEY_INDEX_END',$prmEnd, $prmRangeElem)/@class"/>"</xsl:message-->
        </xsl:otherwise>
    </xsl:choose>
</xsl:function>
...
<xsl:function name="ahf:findIndextermEndFromTopicRef" as="xs:boolean">
    ...
    <xsl:variable name="topicContent" select="if (string($id)) then key('topicById', $id, $root) else ()" as="element()?"/>
