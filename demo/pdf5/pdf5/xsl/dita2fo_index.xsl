...
<!-- map/topicref -->
<xsl:template match="*[contains(@class, ' map/topicref ')][@href]" mode="MAKE_INDEX_ORIGIN">
    ...
    <xsl:variable name="linkTopic" select="if (string($id)) then key('topicById',$id) else ()" as="element()?"/>
        ...
        <xsl:variable name="tempSortKey" as="xs:string">
            <xsl:choose>
                <xsl:when test="not(string($indexSortasText))">
                    <xsl:value-of select="lower-case($indextermText)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$indexSortasText"/>
                </xsl:otherwise>
...
<xsl:template name="dumpIndexterm">
    <xsl:for-each select="$indextermSorted/index-data">
        ...
        <xsl:message>[dumpIndexterm]   id='<xsl:value-of select="generate-id()"/>'</xsl:message>
        ...
        <xsl:message>[dumpIndexterm]   indexsortkey='<xsl:value-of select="text()"/>'</xsl:message>
