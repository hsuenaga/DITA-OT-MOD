...
<!--
 function:	Get target contents as inline
 param:		none
 return:	fo:inline
 note:		function version
 -->
<xsl:template name="ahf:getContentsAsInlineFo" as="node()*">
    <xsl:param name="prmElement" as="element()"/>
    <xsl:apply-templates select="$prmElement" mode="GET_CONTENTS"/>
</xsl:template>
...
<xsl:template name="processIndextermInMetadata">
    ...
    <xsl:apply-templates select="$prmTopicRef/child::*[contains(@class, ' map/topicmeta ')]//*[contains(@class, ' topic/indexterm ')]">
        <xsl:with-param name="prmTopicRef" select="()"/>
        <xsl:with-param name="prmNeedId" select="false()"/>
    ...
    <xsl:apply-templates select="$prmTopicContent/child::*[contains(@class, ' topic/prolog ')]/child::*[contains(@class, ' topic/metadata ')]//*[contains(@class, ' topic/indexterm ')]">
        <xsl:with-param name="prmTopicRef" select="$prmTopicRef"/>
        <xsl:with-param name="prmNeedId" select="false()"/>
        <xsl:with-param name="prmMakeKeyAndStart" tunnel="yes" select="true()"/>
    </xsl:apply-templates>
...
<xsl:template name="processIndextermInMetadataEnd">
    ...
    <fo:wrapper>
    <!--fo:block>
    <xsl:copy-of select="ahf:getAttributeSet('atsIndexDummyBlock')"/-->
    ...
    <xsl:if test="exists($prmTopicContent)">
        <xsl:apply-templates select="$prmTopicContent/descendant-or-self::*[contains(@class,' topic/topic ')]/child::*[contains(@class, ' topic/prolog ')]/child::*[contains(@class, ' topic/metadata ')]//*[contains(@class, ' topic/indexterm ')]">
            <xsl:with-param name="prmTopicRef" select="$prmTopicRef"/>
            <xsl:with-param name="prmNeedId" select="false()"/>
            <xsl:with-param name="prmMakeEnd" tunnel="yes" select="true()"/>
        </xsl:apply-templates>
    ...
    <!-- Make fo:index-range-end FO object for topicref/topicmeta/keywords/indexterm 
         that has @start but has no corresponding @end indexterm
     -->
    <xsl:if test="string($prmTopicRef/@href)=string($lastTopicRef/@href)">
        <xsl:for-each select="$map/descendant::*[contains(@class,' map/topicref ')][@href]">
            <xsl:variable name="topicRef" select="."/>
            <xsl:apply-templates select="$topicRef/child::*[contains(@class, ' map/topicmeta ')]//*[contains(@class, ' topic/indexterm ')]">
